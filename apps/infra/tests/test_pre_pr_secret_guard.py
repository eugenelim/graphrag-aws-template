...
def test_real_email_flagged() -> None:
    assert not hook._is_placeholder_email("real.person@corp.invalid")
...
def test_real_arn_flagged() -> None:
    assert not hook._is_placeholder_arn("000000000000")


def test_scan_flags_real_email() -> None:
    findings = hook._scan_config_text(
        "config.env", ': "${BUDGET_EMAIL:=real.person@corp.invalid}"\n'
    )
    assert findings and "real.person@corp.invalid" in findings[0]


def test_scan_flags_real_role_arn() -> None:
    findings = hook._scan_config_text(
        "config.prod.env", "# INVOKER_ROLE_ARN=arn:aws:iam::000000000000:role/Admin\n"
    )
    assert findings and "000000000000" in findings[0]
