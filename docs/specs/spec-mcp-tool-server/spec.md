...
- Technical: `graphrag.mcp` lives in `packages/graphrag/src/graphrag/mcp/`; files: `__init__.py`, `_tools.py`, `_lambda.py`, `_mock.py`, `_production.py`, `_schemas.py`, `__main__.py`; tests in `packages/graphrag/tests/mcp/`.
...
- Technical: The mock server uses `RuleQueryRouter` only — no Bedrock call is issued in the mock path. `get_policies` in the mock routes directly to the `NormativeRetriever` substitute (`rdflib` named-graph SPARQL), bypassing the router. Production dispatch (ini-002 interim): `_lambda.py` routes to `_production.py::init_production()` when `NEPTUNE_SPARQL_ENDPOINT` is set, otherwise falls back to `init_mock()` with a WARNING. Production tool dispatch uses `NeptuneSparqlStore.sparql_select()` for `get_policies`, `search_graph`, and `query`; `search`, `ask`, and `summarize` use the vector store and embedder (same interface as mock). Production `ask` and `summarize` return a `[RuleQueryRouter]` placeholder until `graphrag.routing` ships. Verified by `tests/mcp/test_production_init.py` (offline, MemorySparqlStore-backed). The full routing contract will be owned by `spec-multi-strategy-routing`.
