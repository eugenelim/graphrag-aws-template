...
- **Assert every load-bearing security invariant from all 4 build specs:**
  ...
  - OpenSearch: domain name `graphrag-vectors`, no AllPrincipals in `access_policies`
    JSON (scoped assertion — not a whole-plan scan), 3 named role principals,
    `es:ESHttp*` scoped to `domain/graphrag-vectors/*`.
...
- [ ] **AC1 — `apps/infra-tf/tests/test_plan.py` exists with ≥23 test functions
  covering all load-bearing invariants.** *(goal-based check)*
  Test count: `pytest --collect-only -q apps/infra-tf/tests/` shows ≥ 23 collected.
...
- [ ] **AC3 — All plan-assertion tests pass against the fixture plan JSON.** *(goal-based check)*
  ...
  exits 0. The fixture plan JSON is generated by `terraform plan -out=tfplan &&
  terraform show -json tfplan` with `-backend=false` and required var stubs; it
  captures the full resource change set from all 4 build specs.
