...
| SEC-2 | `s3_prefix_list_id` is a format-validated free variable — a footgun (a customer-managed list could contain 0.0.0.0/0) | Concern | SURFACE + DEFER: deliberate CDK+scaffold parity (var mirrors the CDK CfnParameter, deploy.sh resolves the AWS-managed list via describe-managed-prefix-lists). Switching to a `data` source removes a scaffold-established variable — an "Ask first" cross-spec change. Rationale documented in spec Assumptions; hardening tracked as backlog `infra-terraform-s3-prefix-list-data-source`. |
